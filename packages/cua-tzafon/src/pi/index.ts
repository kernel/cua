...
export interface TzafonComputerToolsPiOptions extends TzafonComputerToolsOptions {}

export function createTzafonComputerTools(
	translator: ComputerTranslator,
	opts: TzafonComputerToolsPiOptions = {},
): AgentTool<any, TzafonToolDetails>[] {
	return TZAFON_FUNCTION_TOOLS.map((tool) => ({
		name: tool.name,
		label: tool.name,
		description: tool.description,
		parameters: tool.parameters,
		async execute(_id, params): Promise<AgentToolResult<TzafonToolDetails>> {
			...
			if (result.isError) {
				const message = result.content
					...
					.map((part) => part.text)
					.join("\n")
					.trim() || "Tzafon tool execution failed";
...
export function registerTzafonProvider(): void {
	registerApiProvider({
